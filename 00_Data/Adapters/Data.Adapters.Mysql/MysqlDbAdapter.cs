...
{
    public class MysqlDbAdapter : DbAdapterAbstract<SugarParameter>
    {
        public override DbProvider Provider => DbProvider.MySql;

        //"server=127.0.0.1;database=m_admin2;userid=root;password=123456",
        public override async Task<DataTable> GetDataTable(string connectionString, string commandText, params SugarParameter[] parms)
        {
            ...
            {
                var table = await db.Ado.GetDataTableAsync(commandText, parms);
                return table;
            }
        }
	...
	a.TABLE_ROWS  `Rows`,
	c.column_name TablePrimarkeyName,
CASE
...
CASE
	...
	(select COUNT(*) from Information_schema.COLUMNS c  where   c.COLUMN_KEY='PRI'  and c.table_schema=(	SELECT DATABASE
	())  and c.table_name=a.table_name ) PrimaryKeysNum
FROM
	information_schema.TABLES a
	LEFT JOIN

	( SELECT * FROM Information_schema.COLUMNS c  where   c.COLUMN_KEY='PRI'  and c.table_schema=(	SELECT DATABASE
	())  GROUP BY c.table_name ) c ON a.TABLE_NAME = c.TABLE_NAME
...
WHERE
	...
	())  {tables} ";
            ...
            foreach (DbTable item in list)
            {
                ...
                item.VueType = MysqlDbTypeMap.VueCsharpType(item.TablePrimarkeyType);
...
WHERE
	TABLE_NAME = '{tableName}' and table_schema=(select database()) order by ORDINAL_POSITION  ";
            ...
            foreach (var item in list)
            {
                ...
                item.VueStr = EntityJudge.ToLower(item.ColumnName) + ": " + MysqlDbTypeMap.VueCsharpType(item.ColumnType);
                item.VueFromDefault = EntityJudge.ToLower(item.ColumnName) + ": " + MysqlDbTypeMap.VueDefault(item.ColumnType);
                //if ((item.ColumnName=="Id" && item.CShareType == "string")
                //{
