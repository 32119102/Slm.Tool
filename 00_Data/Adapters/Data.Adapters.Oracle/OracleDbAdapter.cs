...
{
    public class OracleDbAdapter : DbAdapterAbstract<SugarParameter>
    {
        ...
        public override async Task<DataTable> GetDataTable(string connectionString, string commandText, params SugarParameter[] parms)
        {
            ...
            try
            {
                using (SqlSugarClient db = new SqlSugarClient(new ConnectionConfig()
                {
                    ...
                    DbType = SqlSugar.DbType.Dm, //数据库类型
...
{tables} ";
            ...
            foreach (DbTable item in list)
            {
                try
                {
                    ...
                    if (item.TablePrimarkeyType != null) {
                        item.CShareType = OracleDbTypeMap.MapCsharpType(item.TablePrimarkeyType, item.Precision, item.Scale);
                        item.VueType = OracleDbTypeMap.VueCsharpType(item.TablePrimarkeyType, item.Precision, item.Scale);
                    }
    ...
    from user_tab_columns a left join user_col_comments b on a.TABLE_NAME = b.table_name and a.COLUMN_NAME = b.column_name
     where a.table_name = '{tableName}' order by column_id ";
            ...
            foreach (var item in list)
            {
                ...
                item.VueStr = EntityJudge.ToLower(item.ColumnName) + ":" + OracleDbTypeMap.VueCsharpType(item.ColumnType, item.Precision, item.Scale);
            }
            return list;
        }
