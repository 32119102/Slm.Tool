...
{
    public class SqlServerDbAdapter : DbAdapterAbstract<SugarParameter>
    {
        ...
        public override async Task<List<DbTable>> GetDbTables(string connectionString, string tables = "")
        {
            ...
            foreach (DbTable item in list)
            {
                ...
                item.VueType = SqlServerDbTypeMap.VueCsharpType(item.TablePrimarkeyType);
        ...
        public override async Task<List<DbColumn>> GetDbColumns(string connectionString, string tableName, DbTable table)
        {
            ...
            foreach (var item in list)
            {
                ...
                item.VueStr = EntityJudge.ToLower(item.ColumnName) + ":" + SqlServerDbTypeMap.VueCsharpType(item.ColumnType);
            }
            return list;
        }
